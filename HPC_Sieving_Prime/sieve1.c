...
{
    unsigned long long low_value = 3 + 2 * ((pid * ((n - 3) / 2 + 1)) / pnum); // the smallest value handled by this process
    unsigned long long high_value = 3 + 2 * (((pid + 1) * ((n - 3) / 2 + 1)) / pnum) - 2; // the largest value handled by this process
